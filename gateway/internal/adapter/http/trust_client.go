...
func (c *TrustClient) EvaluateTrust(ctx context.Context, userID string, roles []string, rc cases.RequestCtx, register bool) (float64, string, map[string]entities.Signal, error) {
	body := map[string]any{
	...
	var result struct {
		...
		Decision   string                     `json:"decision"`
		Signals    map[string]entities.Signal `json:"signals"`
	}
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return 0, "", nil, err
	}

	return result.TrustScore, result.Decision, result.Signals, nil
}
