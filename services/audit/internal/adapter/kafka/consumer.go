...
// Consumer implements port.EventConsumer.
...
// or Close called).
func (c *Consumer) Subscribe(ctx context.Context, topics []string) (<-chan port.Message, error) {
	ch := make(chan port.Message, 256)
	...
	for _, topic := range topics {
		...
		go func(reader *kfk.Reader) {
			...
			for {
				...
				select {
				case ch <- port.Message{Topic: msg.Topic, Value: msg.Value}:
				case <-ctx.Done():
					return
				}
			}
