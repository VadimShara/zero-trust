...
type AuthorizeCase struct {
	...
	idp       port.IDPAdapterService
	clientID  string // expected CLIENT_ID env value
}

func NewAuthorizeCase(sessions port.SessionStore, trust port.TrustService, idp port.IDPAdapterService, clientID string) *AuthorizeCase {
...
func (c *AuthorizeCase) Execute(ctx context.Context, clientID, codeChallenge, method, state string, rc port.RequestCtx) (loginURL string, err error) {
