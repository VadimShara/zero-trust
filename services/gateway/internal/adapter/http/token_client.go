...
var _ port.TokenService = (*TokenClient)(nil)
...
func (c *TokenClient) Issue(ctx context.Context, userID string, roles []string, trustScore float64) (string, string, error) {
	body := map[string]any{"user_id": userID, "roles": roles, "trust_score": trustScore}
...
func (c *TokenClient) Introspect(ctx context.Context, token string) (bool, string, []string, float64, error) {
	...
	return result.Active, result.UserID, result.Roles, result.TrustScore, nil
}

func (c *TokenClient) Refresh(ctx context.Context, refreshToken string, rc port.RequestCtx) (string, string, error) {
	body := map[string]any{
		"refresh_token": refreshToken,
		"ip": rc.IP, "user_agent": rc.UserAgent, "fingerprint": rc.Fingerprint,
	}
	// Don't use c.post: we need to inspect 401 bodies for reuse detection.
